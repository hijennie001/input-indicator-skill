...
- `scripts/qidong_zhishipai.bat` — 同上，纯英文文件名版本（推荐用于制作桌面快捷方式，避免中文路径在部分环境下解析异常）

### 运行方式

1. 双击 `scripts/启动指示牌.bat`（或 `qidong_zhishipai.bat`），或
2. 双击 `scripts/input_indicator.pyw`，或
3. 命令行运行（**注意要 cd 到 `scripts/` 目录再执行**，否则 `pythonw` 找不到文件也不会报错，看起来像“没启动”）：
   ```bash
   cd scripts
...
1. 确认用户是 Windows 系统
2. 将 `scripts/` 下的文件复制到用户想要的位置
3. 告诉用户双击 `.bat` 即可运行
4. 如需桌面快捷方式：用 PowerShell `WScript.Shell` 的 `CreateShortcut` 创建 `.lnk`，`TargetPath` 指向 `qidong_zhishipai.bat`，`WorkingDirectory` 指向脚本所在目录
5. 如需开机自启，可将该快捷方式放入启动文件夹（`shell:startup`）
